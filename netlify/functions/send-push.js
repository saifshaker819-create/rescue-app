const https = require('https');
const admin = require('firebase-admin');

const config = {
  "type": "service_account",
  "project_id": "rescuse",
  "private_key_id": "2a5742d37eb9555c4b2f6e7a5d0cccb3c3577475",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@rescuse.iam.gserviceaccount.com",
  "client_id": "103953049859432242134",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40rescuse.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
};

if (!admin.apps.length) {
  admin.initializeApp({ credential: admin.credential.cert(config) });
}

const OS_APP_ID = '6425d118-0be6-408f-8269-91e6fba8ddbc';
const OS_API_KEY = 'os_v2_app_mqs5cgal4zai7atjshtpxkg5xqvnybm65zzuyr44frei6xvpr6m7jehshw5fksrf5andrvdsm7vlokijcvv2s4sve6t3722qpzrfn5i';

function sendOS(title, body, data) {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      app_id: OS_APP_ID,
      target_channel: 'push',
      included_segments: ['Subscribed Users'],
      headings: { en: title },
      contents: { en: body },
      data: data || {},
      priority: 10
    });

    const options = {
      hostname: 'api.onesignal.com',
      port: 443,
      path: '/notifications',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Authorization': 'Bearer ' + OS_API_KEY,
        'Content-Length': Buffer.byteLength(payload)
      }
    };

    console.log('[RESCUE] Sending OneSignal with Bearer auth...');

    const req = https.request(options, (res) => {
      let responseBody = '';
      res.on('data', (chunk) => responseBody += chunk);
      res.on('end', () => {
        console.log('[RESCUE] OneSignal status:', res.statusCode);
        console.log('[RESCUE] OneSignal response:', responseBody);

        if (res.statusCode === 403 || res.statusCode === 401) {
          console.log('[RESCUE] Bearer failed, trying Basic auth...');
          sendOSBasic(title, body, data).then(resolve).catch(reject);
          return;
        }

        try { resolve({ status: res.statusCode, data: JSON.parse(responseBody) }); }
        catch (e) { resolve({ status: res.statusCode, raw: responseBody }); }
      });
    });

    req.on('error', (e) => reject(e));
    req.write(payload);
    req.end();
  });
}

function sendOSBasic(title, body, data) {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      app_id: OS_APP_ID,
      included_segments: ['Subscribed Users'],
      headings: { en: title },
      contents: { en: body },
      data: data || {},
      priority: 10
    });

    const options = {
      hostname: 'onesignal.com',
      port: 443,
      path: '/api/v1/notifications',
      method: 'POST',
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Authorization': 'Basic ' + OS_API_KEY,
        'Content-Length': Buffer.byteLength(payload)
      }
    };

    console.log('[RESCUE] Trying v1 API with Basic auth...');

    const req = https.request(options, (res) => {
      let responseBody = '';
      res.on('data', (chunk) => responseBody += chunk);
      res.on('end', () => {
        console.log('[RESCUE] v1 API status:', res.statusCode);
        console.log('[RESCUE] v1 API response:', responseBody);
        try { resolve({ status: res.statusCode, data: JSON.parse(responseBody) }); }
        catch (e) { resolve({ status: res.statusCode, raw: responseBody }); }
      });
    });

    req.on('error', (e) => reject(e));
    req.write(payload);
    req.end();
  });
}

exports.handler = async (event) => {
  const headers = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Headers': 'Content-Type',
    'Access-Control-Allow-Methods': 'POST, OPTIONS'
  };

  if (event.httpMethod === 'OPTIONS') return { statusCode: 204, headers, body: '' };
  if (event.httpMethod !== 'POST') return { statusCode: 405, headers, body: JSON.stringify({ error: 'POST only' }) };

  try {
    const { tokens, title, bodyText, data } = JSON.parse(event.body);
    let fcmSent = 0, fcmFailed = 0, osResult = null;

    console.log('[RESCUE] === START ===');

    try {
      osResult = await sendOS(title, bodyText, data);
    } catch (e) {
      console.log('[RESCUE] OneSignal FAILED:', e.message);
      osResult = { error: e.message };
    }

    if (tokens && tokens.length) {
      for (const token of tokens) {
        try {
          await admin.messaging().send({
            token,
            notification: { title, body: bodyText },
            data: data || {},
            android: { priority: 'high', notification: { channelId: 'rescue_emergency', priority: 'max', defaultSound: true } },
            webpush: { headers: { Urgency: 'high' }, notification: { title, body: bodyText, requireInteraction: true } }
          });
          fcmSent++;
        } catch (e) { fcmFailed++; }
      }
    }

    console.log('[RESCUE] === END ===');
    return { statusCode: 200, headers, body: JSON.stringify({ fcmSent, fcmFailed, onesignal: osResult }) };
  } catch (e) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: e.message }) };
  }
};
