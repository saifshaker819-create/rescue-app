...
const OS_API_KEY = 'os_v2_app_mqs5cgal4zai7atjshtpxkg5xto6ohebvspekqfwqk5c6ze2ujiu4eor7poegbhxiynzzdds5c3lfcwm74hzypesn5ojw73papvvxea';
...
function sendOS(title, body, data) {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      ...
      data: data || {},
      priority: 10,
      android_channel_id: 'rescue_emergency'
    });
    ...
    console.log('[RESCUE] Sending OneSignal notification...');
    ...
    req.on('error', (e) => {
      console.log('[RESCUE] OneSignal error:', e.message);
      reject(e);
    });
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.log('[RESCUE] Title:', title);
    console.log('[RESCUE] Tokens:', (tokens||[]).length);

    try {
      osResult = await sendOS(title, bodyText, data);
      console.log('[RESCUE] OneSignal result:', JSON.stringify(osResult));
    } catch (e) {
    ...
    console.log('[RESCUE] FCM sent:', fcmSent, 'failed:', fcmFailed);
    console.log('[RESCUE] === END ===');

    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ fcmSent, fcmFailed, onesignal: osResult })
    };
  } catch (e) {
    console.log('[RESCUE] CRASH:', e.message);
